...
**Work With Us sub-sections** (these three, always):
- Services
- Media Partnerships
- Open Roles

Order rules:
- **Navigation, dropdowns, footer, sitemap, sub-page nav** — `Services → Media Partnerships → Open Roles` (the canonical order).
- **Homepage Work With Us hub cards** — `Media Partnerships → Services` (Media on the left/top, Services on the right/bottom). This is a deliberate product call; do not auto-"fix" it back to the canonical order.
...
├── api/
│   └── cron/
│       └── podcast-refresh.js Serverless — Vercel Cron deploy-hook trigger (timing-safe auth)
├── js/
│   ├── main.js               Nav, scroll reveal, lazy video, hero rotation, mobile sliders
│   └── newsletter.js         Klaviyo client-subscriptions submission (extracted from inline)
├── assets/
...
├── manifest.json             Web App Manifest (PWA-style icons + theme colors)
├── vercel.json
├── sitemap.xml               Auto-generated by build.js (lastmod = build date)
├── llms.txt
└── README.md
```

**Build flow:** Edit `sections/`, `templates/`, or `data/site-content.js` → run `npm run build` → outputs pre-rendered HTML to root. Do NOT edit root `.html` files directly. Build also writes `sitemap.xml` from a fixed URL list (with today's date as `<lastmod>`) and appends `?v=<md5>` cache-busting query strings to every `/css/*` and `/js/*` reference in the output (paired with `immutable, max-age=31536000` headers in `vercel.json`).

**Build-time tokens:** Sections may include `{{token}}` placeholders. The build script resolves these from `data/site-content.js` or computes them at build time:
- Content (from `site-content.js`): `{{podcast_guest_tiles}}`, `{{podcast_recent_cards}}`, `{{podcast_platform_buttons}}`, `{{footer_social_buttons}}`, `{{footer_podcast_links}}`, `{{find_us_icons}}`, `{{learn_free_rows}}`
- Computed at build time: `{{footer_year}}` (current calendar year)

To add/edit guest tiles, free-learning rows, or platform links, edit `data/site-content.js` — not the HTML partials.
...
The four primary "feature cards" — `premium-card-ai`, `premium-card-ue` (on `/education`) and `wwu-card-media`, `wwu-card-services` (on `/work-with-us`) — share **one** visual system. Don't drift them apart.

**Shared shape:**
- Background: flat `var(--color-soot)`. **No atmospheric gradient washes** (deep teal / deep brown / etc.) on these cards. Atmospheric treatments are for full sections; feature cards stay flat so the video does the visual lifting.
- Layout: embedded **16:9 video at the top** (`.premium-card-video-frame` / `.wwu-card-video-frame`), then title → copy → tag chips → CTA below. No full-bleed video backgrounds.
- Desktop dimensions: `min-height: 580px`, `padding: 48px 40px`.
- Mobile dimensions: `min-height: 420px` at ≤1024px, `400px` at ≤640px, `padding: 32px 24px`.
- Hover: `translateY(-4px)` with an accent-tinted border + soft shadow.

**Per-card accent identity (yellow ↔ peach):**
- **Left/top card → yellow accent.** AI Program (Education), Media Partnerships (WWU). Yellow CTA bg, yellow tag chips, yellow-tinted hover border.
- **Right/bottom card → peach accent.** Unreal Engine (Education), Services (WWU). Peach CTA bg with cream text, peach tag chips, peach-tinted hover border.

**Class naming:** kebab-case throughout. Use `.wwu-card-services` / `.wwu-card-media` (not `--services` / `--media`). The legacy BEM `__/--` syntax is gone from these card systems — match it.
...
**Mobile (below 768px):** 56px bar with burger → full-screen overlay. Top-level entries are clickable parent links (`mobile-row-link`) that go to the hub page; their child links sit under them in an always-expanded `.mobile-panel--standalone` panel. No accordion / `<details>` — both **Education** and **Work With Us** use the same shape. Closing on link-click, Escape, or resize above 768px.
...
- `api/cron/podcast-refresh.js` is `GET`-only (returns 405 otherwise) and validates the `Authorization: Bearer <CRON_SECRET>` header with `crypto.timingSafeEqual`. Don't switch this back to plain `===` — it's a defense against timing-attack secret enumeration.
...
- Open Graph + Twitter Card meta on all pages — `og:image` and `twitter:image` use **absolute URLs** (`https://www.baddecisions.studio/og-image.png`), not relative paths. Some unfurlers (Twitter, LinkedIn) reject relative `og:image`.
- `og-image.png` lives at repo root: 1200×630, BD wordmark on brand yellow, ≤300 KB. Same lockup is rasterized down to 180/192/512 for `apple-touch-icon.png` + `icon-192.png` + `icon-512.png`.
- `manifest.json` at repo root declares the Android/PWA install icons (any + maskable purposes), brand `background_color: #FFEF7B`, dark `theme_color: #050505`. Every template links it via `<link rel="manifest">`.
- JSON-LD: `Organization` + `WebSite` (home), `PodcastSeries` (podcast), `Course` with `offers` + `hasCourseInstance` (education), `Service` + `OfferCatalog` (services).
- Sitemap at `/sitemap.xml` is **regenerated by `build.js` on every build** (URL list lives in `buildSitemap()`; `<lastmod>` is today's date). Don't hand-edit the file at root — it's overwritten.
- `llms.txt` at root — keep numbers and cadence in sync with the marketing pages (it's content the LLMs see).
- Skip-nav link with `#main-content` target
- `prefers-reduced-motion` support (CSS + JS, including hero word rotation which also pauses on `visibilitychange` to save battery on backgrounded tabs)
- Local font preloads with `font-display: swap`
- Lazy video autoplay via IntersectionObserver
- All public pages indexed; only the 404 carries `noindex,follow`.
...
- Variant on a card/component: append after another hyphen — `wwu-card-services`, `wwu-card-media`, `or-status-open`. **No BEM `__` or `--` syntax.** A small number of true BEM-style hyphenated state modifiers may exist as a transitional concession (`mp-platform-icon--show` is the lone documented holdout); don't introduce new ones.
